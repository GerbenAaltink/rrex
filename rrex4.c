...
int main() {
    ...
    RBENCH(1, {
        ...
        assert(r4_match("123", "[0-2][2-2][1-3]$"));
        assert(r4_match("aaaabC5", "(a)(\\w)a*(a)\\w[A-Z][0-9]$"));
        assert(r4_match("abcdeeeeee", "ab(cdeee)e"));
        assert(r4_match("1234567", "12(.*)67$"));
        assert(r4_match("12111678993", "12(.*)67(.*)3$"));
        assert(r4_match("NL17RABO0322309700", "NL(.*)R(.*)0(.*)0(.*)0$"));

        assert(r4_match("NL18RABO0322309700",
                        "(\\w{2})(\\d{2})(\\w{4}\\d)(\\d+)$"));
        assert(r4_match("NL18RABO0322309700garbage",
                        "(\\w{2})(\\d{2})(\\w{4}\\d)(\\d+)"));
        assert(r4_match("NL18RABO0322309700",
                        "(\\w{2})(\\d{2})(\\w{4}\\d)(\\d+)$"));
        assert(r4_match(" NL18RABO0322309700",
        ...
        assert(r4_match("  NL18RABO0322309700",
                        "(\\w{2})(\\d{2})(\\w{4}\\d)(\\d+)$"));
        assert(r4_match("NL18RABO0", "(\\w\\w)(\\d\\d)(\\w\\w\\w\\w\\d)$"));
        assert(r4_match("q", "\\q$"));
        r4_t *r =
        ...
        r4_free(r);
        assert(r4_match("ab123", "[a-z0-9]+$"));
        assert(r4_match("ppppony", "p*pppony"));
        assert(r4_match("aa", "a{2}$"));
        assert(r4_match("A23", "[0-2A-z][2-2][1-3]$"));
        assert(r4_match("z23", "[0-2A-z][2-2][1-3]$"));
        assert(r4_match("r23", "[0-2Ar][2-2][1-3]$"));
        assert(r4_match("test", "\\w\\w\\w\\w$"));
        assert(!r4_match("test", "\\W\\w\\w\\w$"));
        assert(r4_match("1est", "\\W\\w\\w\\w$"));
        assert(r4_match("1est", "\\d\\w\\w\\w$"));
        assert(r4_match("Aest", "\\D\\w\\w\\w$"));
        assert(r4_match("abc", "[ab]+"));
        assert(!r4_match("abc", "[ab]+$"));
        assert(r4_match("abc", "[abc]+$"));
        assert(!r4_match("a", "[^ba]"));
        assert(!r4_match("a", "[^ab]"));
        assert(r4_match("                     ponyyzd", "p+o.*yyzd$$$$"));
        assert(r4_match("abc", "def|gek|abc"));
        assert(!r4_match("abc", "def|gek|abd"));
        assert(r4_match("abc", "def|abc|def"));
        assert(r4_match("suwv",
